import * as pg from "./schema-postgres";

/**
 * PostgreSQL-only schema — app imports tables from "@/lib/db/schema".
 */
export const sites = pg.sites;
export const pages = pg.pages;
export const pageBlocks = pg.pageBlocks;
export const pageRevisions = pg.pageRevisions;
export const settings = pg.settings;
export const galleryImages = pg.galleryImages;
export const menuCategories = pg.menuCategories;
export const menuItems = pg.menuItems;
export const bookings = pg.bookings;
export const messages = pg.messages;
export const media = pg.media;
export const adminUsers = pg.adminUsers;
export const passwordResetTokens = pg.passwordResetTokens;
export const userFeatures = pg.userFeatures;
export const navLinks = pg.navLinks;
export const homeSections = pg.homeSections;
export const activityLogs = pg.activityLogs;
export const products = pg.products;
export const sellerApplications = pg.sellerApplications;
export const sellerOrganizations = pg.sellerOrganizations;
export const sellerStores = pg.sellerStores;
export const sellerAccounts = pg.sellerAccounts;
export const sellerMembers = pg.sellerMembers;
export const sellerInvitations = pg.sellerInvitations;
export const blogPosts = pg.blogPosts;
export const serviceCategories = pg.serviceCategories;
export const services = pg.services;
export const carts = pg.carts;
export const cartItems = pg.cartItems;
export const promotions = pg.promotions;
export const promotionRedemptions = pg.promotionRedemptions;
export const paymentConfigurations = pg.paymentConfigurations;
export const orders = pg.orders;
export const orderItems = pg.orderItems;
export const rateLimitBuckets = pg.rateLimitBuckets;
export const customers = pg.customers;
export const customerAddresses = pg.customerAddresses;
export const wishlists = pg.wishlists;
export const recentlyViewedProducts = pg.recentlyViewedProducts;
export const productReviews = pg.productReviews;
export const loyaltyLedger = pg.loyaltyLedger;
export const newsletterSubscribers = pg.newsletterSubscribers;
export const emailCampaigns = pg.emailCampaigns;
export const emailJobs = pg.emailJobs;
export const plans = pg.plans;
export const subscriptions = pg.subscriptions;
export const webhooks = pg.webhooks;
export const webhookDeliveries = pg.webhookDeliveries;
export const siteLocales = pg.siteLocales;
export const pageTranslations = pg.pageTranslations;
export const blockTranslations = pg.blockTranslations;
export const experiments = pg.experiments;
export const experimentAssignments = pg.experimentAssignments;
export const experimentEvents = pg.experimentEvents;
export const pageViews = pg.pageViews;
export const clickEvents = pg.clickEvents;
export const conversionFunnels = pg.conversionFunnels;
export const errorLogs = pg.errorLogs;
export const aiChunks = pg.aiChunks;

export type Site = pg.Site;
export type Page = pg.Page;
export type PageBlock = pg.PageBlock;
export type PageRevision = pg.PageRevision;
export type GalleryImage = pg.GalleryImage;
export type MenuCategory = pg.MenuCategory;
export type MenuItem = pg.MenuItem;
export type Booking = pg.Booking;
export type Message = pg.Message;
export type Media = pg.Media;
export type AdminUser = pg.AdminUser;
export type PasswordResetToken = pg.PasswordResetToken;
export type UserFeature = pg.UserFeature;
export type NavLink = pg.NavLink;
export type HomeSection = pg.HomeSection;
export type ActivityLog = pg.ActivityLog;
export type Product = pg.Product;
export type SellerApplication = pg.SellerApplication;
export type SellerOrganization = pg.SellerOrganization;
export type SellerStore = pg.SellerStore;
export type SellerAccount = pg.SellerAccount;
export type SellerMember = pg.SellerMember;
export type SellerInvitation = pg.SellerInvitation;
export type BlogPost = pg.BlogPost;
export type Cart = pg.Cart;
export type CartItem = pg.CartItem;
export type Promotion = pg.Promotion;
export type PromotionRedemption = pg.PromotionRedemption;
export type Order = pg.Order;
export type OrderItem = pg.OrderItem;
export type PaymentConfiguration = pg.PaymentConfiguration;
export type ServiceCategory = pg.ServiceCategory;
export type Service = pg.Service;
export type RateLimitBucket = pg.RateLimitBucket;
export type Customer = pg.Customer;
export type CustomerAddress = pg.CustomerAddress;
export type Wishlist = pg.Wishlist;
export type RecentlyViewedProduct = pg.RecentlyViewedProduct;
export type ProductReview = pg.ProductReview;
export type LoyaltyLedgerEntry = pg.LoyaltyLedgerEntry;
export type NewsletterSubscriber = pg.NewsletterSubscriber;
export type EmailCampaign = pg.EmailCampaign;
export type EmailJob = pg.EmailJob;
export type Plan = pg.Plan;
export type Subscription = pg.Subscription;
export type Webhook = pg.Webhook;
export type WebhookDelivery = pg.WebhookDelivery;
export type SiteLocale = pg.SiteLocale;
export type PageTranslation = pg.PageTranslation;
export type BlockTranslation = pg.BlockTranslation;
export type Experiment = pg.Experiment;
export type ExperimentAssignment = pg.ExperimentAssignment;
export type ExperimentEvent = pg.ExperimentEvent;
export type PageView = pg.PageView;
export type ClickEvent = pg.ClickEvent;
export type ConversionFunnel = pg.ConversionFunnel;
export type ErrorLog = pg.ErrorLog;
export type AiChunk = pg.AiChunk;

export type NewBooking = pg.NewBooking;
export type NewSite = pg.NewSite;
export type NewPage = pg.NewPage;
export type NewPageBlock = pg.NewPageBlock;
