...
export function AddToCartButton({ productId, available, selectedOptions }: { productId: number; available: boolean; selectedOptions?: Record<string, string> }) {
  ...
  function addToCart() {
    startTransition(async () => {
      try {
        const result = await addStoreCartItem(window.localStorage.getItem("store-cart-token"), productId, selectedOptions);
