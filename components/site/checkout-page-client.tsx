  ...
  return (
    <form action={submit} className="mx-auto grid max-w-6xl gap-8 px-4 py-12 sm:px-6 lg:grid-cols-[1fr_20rem]">
      ...
      <aside className="h-fit rounded-xl border bg-muted/20 p-5">
        ...
        <Button type="submit" className="mt-5 w-full" disabled={pending || !provider}>{pending && <Loader2 className="mr-2 size-4 animate-spin" />}{selectedMethod?.id === "qr" ? "Submit payment reference" : "Place order"}</Button>
      </aside>
    </form>
  );
}
