...
export function SellerManager({ applications, sellers }: { applications: SellerApplication[]; sellers: Seller[] }) {
  ...
  return <div className="space-y-6">
    ...
    <Card><CardHeader><CardTitle>Applications</CardTitle><CardDescription>{applications.length} submitted application{applications.length === 1 ? "" : "s"}.</CardDescription></CardHeader><CardContent className="space-y-4">{applications.length === 0 ? <p className="rounded-lg border border-dashed py-10 text-center text-sm text-muted-foreground">No seller applications yet. Add `/sell` to this site&apos;s navigation when you are ready to recruit sellers.</p> : applications.map((application) => <article key={application.id} className="rounded-xl border p-4 sm:p-5"><div className="flex flex-wrap items-start justify-between gap-3"><div><h3 className="font-semibold">{application.businessName}</h3><p className="text-sm text-muted-foreground">{application.contactName} · {application.email} · {application.country}</p></div><Badge variant={application.status === "approved" ? "default" : application.status === "rejected" ? "destructive" : "secondary"}>{application.status}</Badge></div><p className="mt-4 text-sm leading-6 text-muted-foreground">{application.description}</p>{application.website && <a href={application.website} target="_blank" rel="noreferrer" className="mt-2 inline-block text-sm text-primary hover:underline">{application.website}</a>}{application.status === "pending" ? <div className="mt-4 space-y-3 border-t pt-4"><Textarea value={notes[application.id] ?? ""} onChange={(event) => setNotes((current) => ({ ...current, [application.id]: event.target.value }))} placeholder="Review notes (required for rejection)" maxLength={1000} /><div className="flex flex-wrap gap-2"><Button disabled={pending} onClick={() => review(application, "approved")}><Check />Approve and create store</Button><Button disabled={pending} variant="destructive" onClick={() => review(application, "rejected")}><X />Reject</Button></div></div> : application.reviewNotes && <p className="mt-4 rounded-lg bg-muted p-3 text-sm">{application.reviewNotes}</p>}</article>)}</CardContent></Card>
    {sellers.length > 0 && <Card><CardHeader><CardTitle>Verified sellers</CardTitle><CardDescription>Approved organizations and their initial stores.</CardDescription></CardHeader><CardContent className="grid gap-3 sm:grid-cols-2">{sellers.map((seller) => <div key={seller.id} className="rounded-lg border p-4"><div className="flex items-start justify-between gap-3"><div><p className="font-semibold">{seller.name}</p><p className="text-sm text-muted-foreground">{seller.contactEmail}</p></div><Badge>{seller.status}</Badge></div>{seller.storeName && <p className="mt-3 text-sm"><Store className="mr-1 inline size-4 text-primary" />{seller.storeName} <span className="text-muted-foreground">/{seller.storeSlug}</span></p>}</div>)}</CardContent></Card>}
  </div>;
}
