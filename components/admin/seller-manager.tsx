...
import { resendSellerInvitation, reviewSellerApplication } from "@/lib/actions/marketplace";
...
export function SellerManager({ applications, sellers }: { applications: SellerApplication[]; sellers: Seller[] }) {
  ...
  function resendInvitation(seller: Seller) { startTransition(async () => { try { await resendSellerInvitation(siteId, seller.id); toast.success(`Invitation sent to ${seller.contactEmail}.`); } catch (error) { toast.error(error instanceof Error ? error.message : "Unable to send invitation."); } }); }
